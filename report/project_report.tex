...
The main idea behind this network is the implementation of residual learning. In the previous models, the exact copy of the input has to go through all layers until it reaches the output layer. This becomes an end-to-end relation between input  and output and in a deep model creates serious problmes such as difficult to train the network and vanishing/exploding gradients.
...
After the generation of the output is possible reconstruct the HR image summing the interpolated image with the residual.
